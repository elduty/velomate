...
class TestFitMonodScherrer:
    ...
    def test_negative_cp_rejected_as_failed(self):
        ...
        # Constructed so polyfit produces a negative intercept
    ...
    def test_negative_w_prime_rejected_as_failed(self):
        """Inverted relationship (P decreases as 1/t increases) → negative slope."""
        # P drops as duration shortens — physically impossible
        efforts = [(60, 100.0), (120, 150.0), (300, 200.0), (600, 250.0)]
        cp, w_prime_kj, r2 = fit_monod_scherrer(efforts)
        assert cp is None and w_prime_kj is None and r2 is None
```
...
def compute_cp_estimate(
    ...
    # Always compute the rolling 20-min fallback for comparison + as the
    # fallback chosen value if CP fails the quality gate.
    fallback = estimate_ftp(conn)
...
```python
    ...
    # 20-min × 0.95 when fit quality is poor)
    print("[fitness] Computing CP / W' estimate...")
    try:
        compute_cp_estimate(conn)
...
pd_curve['targets'] = [{
    "rawSql": (
        ...
        ")\n"
        "SELECT 60   AS \"duration_s\", MAX(p60)   AS \"power\" FROM rolling UNION ALL\n"
        "SELECT 120,  MAX(p120)  FROM rolling UNION ALL\n"
        "SELECT 300,  MAX(p300)  FROM rolling UNION ALL\n"
        "SELECT 600,  MAX(p600)  FROM rolling UNION ALL\n"
        "SELECT 1200, MAX(p1200) FROM rolling\n"
        "ORDER BY 1;"
    ),
    "format": "table",
    "refId": "A"
