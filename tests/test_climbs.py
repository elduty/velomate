...
from climbs import smooth_altitude, detect_climbs, classify_climb, _rdp_with_original_indices
...
class TestRDP:
    def test_straight_line_simplified_to_endpoints(self):
        points = [(float(i), float(i)) for i in range(100)]
        result = _rdp_with_original_indices(points, epsilon=1.0)
        assert len(result) == 2
        assert result[0][2] == 0
        assert result[1][2] == 99

    def test_triangle_preserves_peak(self):
        points = [(0.0, 0.0), (50.0, 100.0), (100.0, 0.0)]
        result = _rdp_with_original_indices(points, epsilon=1.0)
        assert len(result) == 3  # all three points preserved

    def test_small_epsilon_preserves_more(self):
        points = [(float(i), 10.0 * math.sin(i * 0.1)) for i in range(100)]
        small = _rdp_with_original_indices(points, epsilon=0.1)
        large = _rdp_with_original_indices(points, epsilon=5.0)
...
class TestDetectClimbs:
    ...
    def test_climb_with_small_dip_merged(self):
        """Two uphills with a small dip — RDP should smooth it out."""
        alt = (
        ...
        climbs = detect_climbs(alt, dist)
        assert len(climbs) >= 1
        total_gain = sum(c["gain_m"] for c in climbs)
        assert total_gain >= 70
